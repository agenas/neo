...
class IndicatorsHolder {
    ...
    static List<String> DESCRIPTOR_INPUT_FIELDS = IndicatorInput.declaredFields.name.findAll { !it.startsWith('$') && !it.startsWith('_') && it != 'metaClass' }
    ...
    void parse(File indicatorsDir, String currentLocale = 'it') {
        if (indicatorsDir.exists()) {
            def selectUnitSpecsFile = new File(indicatorsDir, NeoboxUtils.INDICATORS_SELECTUNIT_FILE)
            ...
            indicatorsDir.eachFile(FileType.DIRECTORIES) { dir ->
                def descriptorFile = new File(dir, NeoboxUtils.INDICATOR_SPECS_FILE)

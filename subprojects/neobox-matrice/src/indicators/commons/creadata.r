...
####################################################################
# Change only this to create a dataset of the desired sample size
n<-175000
####################################################################

####################################################################
# adjust to create testdata_thematrix.csv and testdata_distretti.csv
setwd("/home/fabrizio/gitbiro/neobox/subprojects/neobox-import-specs/src/data")
####################################################################

####################################################################
# H2 commands to access the dataset
#
# program directory:
# cd  /home/fabrizio/gitbiro/neobox/subprojects/neobox-app
# java client:
# java -cp staging/h2-1.3.170.jar org.h2.tools.Server
# dump from client:
# call CSVWRITE ( '/home/fabrizio/Desktop/neobox_run/data/testdata.csv', 'SELECT * FROM THEMATRIX' ) ;
# SQL commands:
...
####################################################################
 ...
 x[,"COMUNE"]<-sample(1:206,size=n,replace=TRUE,prob=c(rep(n/206,206)))
 ...
 ####################################################################
 # Change only this to create a dataset with different location
 x[,"COMUNE"]<-paste("17",x[,"COMUNE"],sep="") # Brescia
 ####################################################################
 ...
 ####################################################################
 # Change only this to create a dataset with different impact of diseases
 ...
 write.csv(x,"testdata_thematrix.csv",quote=FALSE,row.names=FALSE,na="")
 ...
 y[,"MMG"]<-sample(1:n_mmg,size=n_mmg,replace=TRUE,prob=c(rep(1/n_mmg,n_mmg)))
 ...
 write.csv(y,"testdata_distretti.csv",quote=FALSE,row.names=FALSE,na="")

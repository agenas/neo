...
def typesMapping = [
    data:    [type: 'varchar',  defaultSize: 8],
...
def csv = new File('data/tracciato1.csv').withReader('UTF-8') {
    def data = CsvParser.parseCsv(it)

    new File("data/thematrix.specs").withWriter('UTF-8') { w ->
